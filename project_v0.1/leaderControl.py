...
class leaderControl(object):
	...
	def controller(self, x1, x2, y1, y2):
		...
		self.vx = self.vx0 + (x2 - x1) * (currentDisPow - pow(self.d12, 2)) * 0.02  #0.02 is arbitrary scale factor
		self.vy = self.vy0 + (y2 - y1) * (currentDisPow - pow(self.d12, 2)) * 0.02
...
if __name__ == "__main__":
	leader1 = leaderControl(1.0, 1.0, 2.0, 0.0, 0.0)
	leader2 = leaderControl(1.0, 1.0, 2.0, 10.0, 0.0)
	delt_T = 0.5  #s
