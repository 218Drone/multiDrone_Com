...
@author: Peng Cheng
		 Hongliang Shen
		 Xi Zhang 
@date: 2015/11/3    11:10    v0.1: the original version.
...
if myserial.isOpen():
    ...
    '''initialise datas'''
    delt_T_s = 50  #500ms
    delt_T_l = 1000   #1000ms
    lastRecord_s = current_milli_time()   #control loop
    lastRecord_l = current_milli_time()   #data loop
    ...
    #initialize
    neighbourLonToMeter = float(math.pi / 180) * R * vehicle.location.lat
    neighbourLatToMeter = float(math.pi / 180) * R * vehicle.location.lon
    ...
    while not api.exit:
        ...
        if current_milli_time() - lastRecord_l >= delt_T_l:
            lastRecord_l = current_milli_time()
            #write
            myserial.write('o'+str("%.8f" % vehicle.location.lon))
            myserial.write('a'+str("%.8f" % vehicle.location.lat))
